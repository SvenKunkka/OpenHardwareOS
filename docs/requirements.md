...
| A real machine's evidence can be collected and read elsewhere | Implemented (round 13) | `ohm-cli report` writes one JSON file carrying both halves: what this project reports (providers with status and reason, devices with readings, the reason for each missing reading, capability notes, installed rules) and what the platform itself says, unparsed (every `hwmon` chip's `fan*_input`/`pwm*`/`pwm*_enable`/`temp*_input` contents, `/proc/meminfo`, `df -k`). Side by side so a reading can be compared with the file it came from without trusting this project's parser; every unreadable section explains itself. `docs/field-checklist.md` is the procedure: read-only confirmation, restricted control of one channel with its rollback, normal exit and a forced kill (which does not hand control back, recorded as fact), and the list of claims that still have no evidence. **Not** verified: the checklist has never been executed on a real desktop, and the Windows half of the report has never been filled from a machine running LibreHardwareMonitor. |
| The service reports what each rule is doing, and survives the outages | Implemented (round 12) | The state file carries every rule's status, confirmed value and own message, and `service status` prints them — "3 rules loaded" cannot distinguish cooling along the curve from sitting on the fail-safe. `service run` logs into the config directory's `logs/` (a process nobody watches must leave a record) and prints both paths in its banner. `OHM_HWMON_ROOT` points a whole process at another hwmon tree (read-only; this build writes no `pwm<N>`), which is what lets `apps/cli/tests/service_resilience.rs` delete a tachometer under a running service: the rule moves to `fallback` at the fail-safe 70 % naming the sensor that went away, and reads again when the file returns. The same file covers `SIGSTOP`/`SIGCONT`: past the heartbeat window `service status` reports "not running" and names whose stale file it is, and after resuming the same process still owns it with the cycle counter moving. **Not** verified: real hardware, a real kernel suspend, and Windows service integration. |
| Rules keep running with no window open, and one process owns a channel | Implemented (round 11) | `ohm-cli service run` starts the runtime and the rule loop headlessly and `service status` reports from the state file (pid, uptime, heartbeat age, cycles, rules; exit 1 when nothing runs). Channels have **one writer**: the state file is claimed with `create_new`, liveness is a heartbeat (so a file from a `SIGKILL`ed process is recognised as abandoned), takeover is atomic, an unparseable file counts as unowned, and `AutomationEngine::start` refuses to run when another live process holds it — the desktop keeps reading and says why in `stats().blocked_by`, the log and the event feed. `SIGTERM`/`Ctrl-C` (handler installed *before* the claim) stops the loop, shuts the runtime down — which releases control and reports confirmed/unconfirmed/refused/failed separately — and removes the state file. **Not** verified: real hardware, suspend/resume, sensor loss at the service level, and any Windows service integration; `docs/background-service.md` says so and ships a systemd *user* unit template rather than installing anything. |
| Every reading can be checked against the machine's own source | Implemented (round 10) | `ohm-cli status --json` / `doctor --json` print one JSON object (the same snapshot the desktop renders from; the paths a script reads are pinned by a test in `crates/ohm-runtime/src/snapshot.rs`). `scripts/verify-linux-readings.sh` compares each reading with the platform on the same host and prints `AGREE` / `DIFFER` (exit 1) / `NO-SOURCE` (with the reason) / `NOT-CHECKED`: memory against `/proc/meminfo`, fan RPM and PWM against the exact hwmon file the reading came from, temperatures against the range the platform reports, free space against `df` for the same mount point. A machine with no fan channels is a normal `NO-SOURCE`, and `cpu.load`/`cpu.frequency` are declared uncheckable from one sample rather than faked. Runs live in CI and in the release pipeline: on ubuntu-latest in v0.1.5 the memory total matched exactly, three mounts matched `df` to 0/0/16384 bytes, and the temperature sat inside the platform's range. Fixtures (`scripts/tests/verify-linux-readings.test.sh`, 6 cases / 21 checks) feed it synthetic trees that agree, disagree in four places, omit a reading with no reason, and have no fan channels at all. |
| Linux desktop package: installable without compiling | Implemented (round 9) | `scripts/release/package-linux.sh` takes `--desktop-deb`/`--desktop-appimage` (both or neither) and publishes `OpenHardwareOS-<version>-linux-x86_64.deb` / `.AppImage` under this project's own names, sharing one checksum list with the CLI archive. Before publishing, `scripts/release/inspect-deb.py` reads the `.deb` (an `ar` container holding `control.tar.*` and `data.tar.*`) and the packager requires this release's version, the package name `open-hardware-os`, the architecture `amd64`, a 64-bit x86_64 ELF at `usr/bin/openhardwareos`, a `.desktop` entry whose `Exec` launches that binary, and icons; the AppImage must be an executable x86_64 ELF. The release run then *installs* the package with `apt-get` on the runner and runs the installed binary (`--selftest --mock`, and once `--dry-run`), then runs the AppImage: [run 34931865680](https://github.com/SvenKunkka/OpenHardwareOS/actions/runs/34931865680) on Ubuntu 24.04.5 (kernel 6.17.0-1022-azure) reports `installed as /usr/bin/openhardwareos`. **Not** verified: the window, tray and menu entry (the headless self-test returns before the GUI is built), a real machine, and non-Debian distributions. `scripts/tests/package-linux.test.sh` is 10 cases / 54 checks with synthetic packages, including deliberately wrong ones. |
| Whether the background rules can reach hardware is answerable before they are left running | Implemented (round 20) | `ohm-cli service check` is a read-only preflight: it prints who owns the channels now (pid, cycles, heartbeat age), whether automation is on and whether this is a dry run, every provider's status with its reason, the cooling channels this configuration confirms **with the platform's own answer about each**, how many rules are enabled, and whether control is handed back on exit — then one verdict about the *configured intent*. `READY` / `READY (READ ONLY)` / `READY (DRY RUN)` exit 0; a channel the configuration confirms that this process cannot write is `NOT READY` and exits 1, because that is the failure nobody can see from outside (healthy banner, moving cycle counter, motionless fan). The answer comes from opening the channel's `pwm<N>` file for writing and closing it again: the kernel refuses when this process is not allowed, and it refuses for a read-only mount or a policy module too, in words a person can act on. `HardwareAdapter::write_access` carries it (default `Unknown`, never a hopeful `Permitted`), the runtime routes to the owning adapter, and `service run` prints the same findings as warnings at start-up instead of being refused once per second in silence. Tests: 5 adapter unit tests, and 6 process-level cases against the real binary and prepared trees (permitted, refused with exit 1, nothing confirmed, dry run, automation off, another service running and left untouched). Mutation-checked: making the platform refusal return `Permitted` fails both the unit and the process-level test. **Not** verified: no real Linux as a non-root user and no real Windows without elevation, so "writes need root/Administrator" is still a claim about those platforms rather than a measurement. |
| A channel switched by a process that died is handed over, so firmware control comes back | Implemented (round 19) | A killed owner never runs its own shutdown, and the value its channel had before it switched exists only in its memory. The state file now carries it — `ServiceState.taken`, filled from `Runtime::taken_controls`, which collects `HardwareAdapter::taken_controls` across adapters (default empty; the hwmon side reports the exact `pwm<N>_enable` number, and `ControlMode::Automatic` keeps it, so `3` — automatic with the driver's curve — is not decayed into `2`). A successor adopts that record **before it writes anything**: it writes the recorded value back to the kernel straight away *and* remembers it as its own original, so its own exit restores the value the dead owner had, not the switched one it found. `adopt_taken_controls` returns one sentence per channel it could not adopt (no restorable value, not in `adapter_settings.system.pwm_write_allow`, channel no longer in the tree) and the service prints them; the runtime adds a net for a channel no adapter claimed. The owner publishes on change rather than on the heartbeat, so "switched but not yet recorded" is a millisecond window — a limit written down in `docs/background-service.md` rather than claimed to be zero. Tests: 5 in `adapters/system` (report, adopt, exact 3, unreadable refused, not-allowed refused), 3 in `crates/ohm-runtime/src/service.rs` (publish, predecessor, older file) and one process-level test with two real services on a prepared tree, which fails when adoption is removed (mutation-checked). **Not** verified: no real Windows/Linux machine, and the residual crash window is not measured. |
| A service that wakes up after somebody else took the channels stands down instead of becoming a second writer | Implemented (round 18) | The heartbeat window is deliberately shorter than a suspend, so while a process is frozen another process may legitimately take over — which is only safe if the woken owner checks. `ServiceGuard::verify_ownership` runs before **every** heartbeat and refuses to write when the file names another pid or is gone, so a rename can no longer replace the new owner's record, and the guard is printed, never overwritten; `Drop` removes the file only when it is still this process's, or it would erase a successor's record and leave its channels claimed by nobody. The service treats both the start-up heartbeat and every loop heartbeat the same way: it logs *"another process took over the channels (pid …)"*, stops its loop, releases what it took, and exits 4 rather than 0. A wall-clock/monotonic disagreement (the signature of a real suspend: wall time keeps moving, the process's own clock does not) is detected separately and recorded as `resumes` / `last_resume_gap_ms` in the state file, because that gap is exactly what a heartbeat cannot show. **Not** verified: a real kernel suspend (the detector's premise is a platform property, tested only as a pure function), and the ownership of firmware control after a takeover — see the known gap in `docs/background-service.md`. Tests: `crates/ohm-runtime/src/service.rs` (10 new: ownership, drop, detector, older files still parsing) and `apps/cli/tests/service_resilience.rs` (2 new, two real processes). |
| Device identity survives a re-enumeration — a rule must not change target because the provider listed devices in another order | Implemented (round 17) | Every id is built from something the provider itself uses to address the hardware, never from our position in a list. The LibreHardwareMonitor adapter used to number devices by traversal position, so a GPU enumerating before the board moved `fan.lhm.0` to a different header — every id still existed and every one of them was writable, which is how such a mismatch stays invisible. It now composes ids from LHM's own path and the channel number LHM reports (`fan.lhm.lpc_nct6687d_0_1`, `motherboard.lhm.lpc_nct6687d_0`), keyed on the hardware path rather than on which of a tachometer/control pair happens to be present, so a sensor that vanishes and returns keeps its id. `DeviceId::compose` remains for devices with nothing better to be named after. Where two nodes cannot be told apart (paths that normalise to the same key) the second is addressed by position **and the adapter reports it** as `Degraded`, alongside channels whose control could not be matched to a physical fan — identity that cannot be established is stated, never presented as certain. **Not** verified: no real Windows machine has run this, so the claim rests on `adapters/libre-hardware-monitor` tests (51) over prepared trees, an adapter-level status test, and one deterministic reordering of the whole fixture tree. |
| Linux sensor sources: hwmon fan tachometers and RAM | Implemented (round 8) | `adapters/system/src/hwmon.rs` reads `/sys/class/hwmon` (`fan<N>_input`, `fan<N>_label`, `pwm<N>`, `pwm<N>_enable`) per channel and exposes `fan.rpm` plus the duty the kernel is applying. The device id is the chip name and channel (`fan.system.nct6798d_fan1`), never an enumeration index, so a kernel that renumbers `hwmon*` cannot move a rule's target; a missing file, non-numeric content, a chip without `name`, a vanished channel and an unknown `pwm<N>_enable` value each carry a reason. RAM is `memory.system.0` (`memory.used`, `memory.total`, bytes) from `sysinfo`. Reading is unconditional. |
| Writing an hwmon channel: per channel, opt-in, and reversible | Implemented (round 14) | `pwm<N>` and `fan<N>_input` share a channel number, which is **not** a promise that they are the same physical header — that pairing is a property of the board, so a person confirms it and lists the channel in `adapter_settings.system.pwm_write_allow` (empty by default; blank and non-string entries match nothing). Only a listed channel gains a writable `fan.speed_percent`; the rest stay read-only, and a write that reaches the path anyway is `Rejected` naming the setting. The first write switches `pwm<N>_enable` to manual and remembers what it was; `shutdown` restores it and reports failure loudly (`AdapterCapabilities::hands_back_control_on_shutdown` is now true for this adapter, `write_requires_admin` too, because writing `pwm<N>` needs root). Ownership that cannot be read — an unreadable file, or a value that is not a number — is **not** taken over and never guessed back. Writes still pass the runtime's safety layer (floor, ramp, emergency) and are audited. **Not** verified on any real machine: 13 tests cover the fake-sysfs path, and `docs/field-checklist.md` §3 is the procedure that would produce the first real record. |
...
| The artefact under test is the one this tree builds, and the round trip that proves it actually runs | Implemented (round 15) | `scripts/verify-ipc-roundtrip.sh` reads the desktop artefact's name from `apps/desktop/src-tauri/tauri.conf.json` instead of assuming it, stops **before** judging anything when the build did not produce that name, and requires the artefact's own `--selftest` version to equal the version the tree declares, so a stale binary carrying the right name cannot stand in for this revision's code. The row exists because the opposite was true for six rounds: `29b95dd` renamed the artefact, the script kept looking for the old path, and — since a missing command also exits non-zero — the two "refused when unisolated" checks above it passed on exit 127 while the round trip never ran. Rounds 9–14 of `docs/verification-log.md` recorded it as verified; those six rows are corrected in place, and round 15 re-verified it. |
| A renamed built artefact, or an install entry pointing at a version nobody can download, fails a test | Implemented (round 16) | `scripts/check-artefact-names.py` treats `mainBinaryName` in `apps/desktop/src-tauri/tauri.conf.json` as the desktop artefact's only source of truth and `apps/cli/Cargo.toml` as the CLI's, requires the Linux install page, the CLI staging/archiving and `install.ps1` to agree with them, refuses a non-comment literal `target/release/<name>` in any shell script (the name must be derived), and requires every install entry — download URL, `$ohmVersion`, `ohm_version`, `cargo install --tag` — to name the newest released version or the one being prepared. `scripts/release/package-linux.sh` now derives the expected binary name from `mainBinaryName` and refuses a `.deb` that installs anything else, which `scripts/tests/package-linux.test.sh` proves by shipping a `.deb` with a differently named binary and by pointing the packager at a checkout that declares another name. `scripts/tests/test_artefact_names.py` (12 tests) mutates each source of truth in a throwaway copy and requires the checker to report it, so the guard cannot pass vacuously. Wired into CI's `Version catalogue and lifecycle` job. **Not** verified: nothing on a real machine — this is a repository-level check. |
