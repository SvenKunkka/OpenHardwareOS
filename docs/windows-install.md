...
CLI 安装在当前账户的 `%LOCALAPPDATA%\OpenHardwareOS\cli-v0.1.0`，不需要管理员权限，
...
```powershell
$env:Path = "$env:LOCALAPPDATA\OpenHardwareOS\cli-v0.1.0;$env:Path"
...
CLI 可通过删除对应的 `%LOCALAPPDATA%\OpenHardwareOS\cli-v0.1.0` 目录卸载。
