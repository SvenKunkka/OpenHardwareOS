...
本页安装 Windows x64 预览版本 `v0.1.6`。预编译安装无需 Rust 或 Node.js。
...
CLI 安装在当前账户的 `%LOCALAPPDATA%\OpenHardwareOS\cli-v0.1.6`，不需要管理员权限，
...
```powershell
$env:Path = "$env:LOCALAPPDATA\OpenHardwareOS\cli-v0.1.6;$env:Path"
...
```powershell
Invoke-WebRequest -UseBasicParsing 'https://github.com/SvenKunkka/OpenHardwareOS/releases/download/v0.1.6/install.ps1' -OutFile "$env:TEMP\OpenHardwareOS-install.ps1"
& "$env:TEMP\OpenHardwareOS-install.ps1" -Version v0.1.6 -Desktop
```
...
```powershell
cargo install --git https://github.com/SvenKunkka/OpenHardwareOS --tag v0.1.6 --locked ohm-cli
ohm-cli --version
```
...
CLI 可通过删除对应的 `%LOCALAPPDATA%\OpenHardwareOS\cli-v0.1.6` 目录卸载。
