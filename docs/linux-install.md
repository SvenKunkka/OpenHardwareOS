...
本页安装 Linux x86_64 预览版本 `v0.1.5`，两种方式：**命令行（CLI）** 与 **桌面应用（.deb）**。
...
把 `~/.local/share/OpenHardwareOS/cli-v0.1.5` 加入 `PATH` 之后，可以直接运行：

```bash
export PATH="$HOME/.local/share/OpenHardwareOS/cli-v0.1.5:$PATH"
